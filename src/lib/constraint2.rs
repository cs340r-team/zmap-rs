...
impl Constraint {
    ...
    fn lookup_ip(&self, root: TreeNodeRef, addr: u32) -> i32 {
        ...
        let mut cur_node: Rc<RefCell<TreeNode>> = root;
        loop {
            {
                //let cur_node: std::cell::Ref<TreeNode> = cur_root.borrow();
                if cur_node.borrow().is_leaf() {
                    return cur_node.borrow().val;
                }
                let next: Option<Rc<RefCell<TreeNode>>> = if addr & mask != 0 {
                ...
                mask >>= 1;
                if mask == 0 {
                    println!("Reached the end! {:?}", cur_node);
                    return 0;
                }
            }
        }
    ...
    fn count_ips_recurse(&self, node: TreeNodeRef, value: i32, size: i64) -> i64 {
        ...
        // in c code the return 9=0 here why?
